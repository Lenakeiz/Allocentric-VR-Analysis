function VisualizeData(AlloData, varargin)
...
arg=1;
    ...
    conds = [];
    for i = 1 : 3
        tmpErr = [AlloData.AbsError(AlloData.ParticipantGroup == 1 & AlloData.TrialType == i & AlloData.ConfigurationType == objectConf)];
        yErr = [yErr;tmpErr];
    ...
    conds = [];
    for i = 1 : 3
        tmpErr = [AlloData.AbsError(AlloData.ParticipantGroup == 2 & AlloData.TrialType == i & AlloData.ConfigurationType == objectConf)];
        yErr = [yErr;tmpErr];
        conds = [conds ; ones(size(tmpErr,1),1) * i + jitterFromUnit];
    end
