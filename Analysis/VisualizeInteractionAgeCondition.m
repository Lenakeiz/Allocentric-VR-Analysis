...
tempData = AlloData_SPSS_Cond_Conf(:,[1 2 4 5]);
%tempData.MeanADE = log(tempData.MeanADE);

for i = 1:2
    for j = 1:3
        tempDataBlock = tempData(tempData.TrialType == j & tempData.ParticipantGroup == i,:);
...
for i = 1:2
    ...
    for ii=1:length(plotone{i})
        ...
        plotone{i}(ii).data.SizeData = 200;
...
clear blocks conds tempData tempDataBlock blockOne blockTwo blockThree ans a meanS stdS ax plotone jitter groupColors ii fh i dataStruct j ...
    ego allow allot aarray dataPlot blocks jitterfromUnit spacing lPlot distanceBetweenUnits leg meanHelmertContrasts stdHelmertContrasts s meanContrasts stdContrasts B configs TF kk