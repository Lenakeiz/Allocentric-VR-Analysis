...
groupwiseMeans = varfun(funcOmitNan, AlloDataBlock, 'InputVariables', 'MeanAbsError', ...
                        'GroupingVariables', {'ParticipantID', 'ConfigurationType'});

% Separate data by participant group
one_object_data = groupwiseMeans.Fun_MeanAbsError(groupwiseMeans.ConfigurationType == 1);
four_object_data = groupwiseMeans.Fun_MeanAbsError(groupwiseMeans.ConfigurationType == 4);

% Combine data into a cell array for ease of plotting
y_data = {one_object_data, four_object_data};
x_categories = {'1-obj', '4-obj'};
...
ylims = [0, 5.5];
hlines = [1,2,3,4,5];
colors = {config.colorPalette.one_object, config.colorPalette.four_object};  % Colors for scatter points
