...
for i = 1:2
    ...
    tempArray = accumarray(tempDataBlock.ParticipantID,tempDataBlock.MeanADE, [], @nanmean);
    tempArray(tempArray==0) = nan;
    tempArray(any(isnan(tempArray),2), :) = [];
    dataStruct{i}{1} = tempArray;
    tempArray = accumarray(tempDataBlock.ParticipantID,tempDataBlock.MeanRT, [], @nanmean);
    tempArray(tempArray==0) = nan;
    tempArray(any(isnan(tempArray),2), :) = [];
    dataStruct{i}{2} = tempArray;
end

%1 is elder, 2 is young, 4 is chance
groupColors = [config.colorPalette.young; config.colorPalette.elderly];
